...
def case5Compresion(data):

    required_fields = ['sistema', 'material', 'C', 'd', 'Do_def', 'Lf_def', 'ymax',
    ...
    try:
        ...
        comp_F_max_calc = Fmax(k_value, ymax)
        comp_F_min_calc = Fmin(k_value, ymin)
        ...
        comp_y_calculado = y_cal(comp_F_max_calc, comp_F_min_calc, k_value)
        comp_Na = Na(comp_D, d, G, k_value)
        comp_k_new = k(comp_D, d, comp_Na, G)
        comp_y_new = y(comp_F_max_calc, comp_D, d, comp_Na, G)
        ...
        comp_yinicial = yinicial(comp_F_min_calc, comp_k_new)
        comp_ychoque = ychoque(comp_y_calculado)
        ...
        comp_Do = Do(comp_D, d)
        comp_Di = Di(comp_D, d)
        comp_Fsolida = Fsolida(comp_k_new, comp_ysolida)
        ...
        result = {
            ...
            'D': comp_D_new,
            'Fmax': comp_F_max_calc,
            'Fmin': comp_F_min_calc,
            'coil_diam': comp_D,
            'Sut': comp_Sut,
            'Sys': comp_Sys,
            'Ks': comp_Ks,
        ...
        fatiga_result = {}
        # Cálculos de fatiga, compresión caso 5.
        if Fatiga:
            fatiga_result = calcular_fatiga_compresion(
                comp_F_max_calc, comp_F_min_calc, C, d, comp_D, comp_Sus, Tratamiento, sistema)
